...
export function BubbleChart({ 
  ...
  // Calculate axis domains with proper padding to prevent bubbles from extending beyond axes
  const { xDomain, yDomain } = useMemo(() => {
    if (transformedData.length === 0) {
      return { xDomain: [0, 10], yDomain: [0, 10] }
    }

    // Get min/max values for both axes
    const cagrValues = transformedData.map(d => d.cagrIndex)
    const marketShareValues = transformedData.map(d => d.marketShareIndex)

    const minCagr = Math.min(...cagrValues)
    const maxCagr = Math.max(...cagrValues)
    const minMarketShare = Math.min(...marketShareValues)
    const maxMarketShare = Math.max(...marketShareValues)

    // Calculate the range and add padding (20% of range on each side to accommodate bubbles)
    const cagrRange = maxCagr - minCagr
    const marketShareRange = maxMarketShare - minMarketShare

    // Use at least 20% padding, or more if range is very small
    const cagrPadding = Math.max(cagrRange * 0.2, 0.5)
    const marketSharePadding = Math.max(marketShareRange * 0.2, 0.5)

    return {
      xDomain: [minCagr - cagrPadding, maxCagr + cagrPadding],
      yDomain: [minMarketShare - marketSharePadding, maxMarketShare + marketSharePadding]
    }
  }, [transformedData])
